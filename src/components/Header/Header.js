				...
				<article className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-center mb-20 p-8 pb-12 max-w-4xl rounded-lg w-full">
					...
					<RubberBand delay={1000}>
						<h1 className="text-lg font-bold font-firamono text-left md:text-3xl mt-2.5 drop-shadow">
							...
							<span
								id="name"
								className="text-4xl lg:text-6xl mb-1 drop-shadow md:drop-shadow-lg text-transparent bg-clip-text bg-gradient-to-t from-rose-400 to-indigo-500"
							>
								CHRISTY LE.
							</span>
						</h1>
					...
					<a
						...
						className="bg-transparent bg-gradient-to-r hover:from-indigo-400 hover:to-rose-400 hover:text-white border hover:border-0 border-rose-600 hover:border-transparent hover:animate-pulse rounded-xl p-3 text-rose-600 text-md md:text-xl font-bold px-10 drop-shadow"
					>
						GET IN TOUCH
					</a>
				</article>
				{/* <p className="bg-fixed animate-bounce text-black absolute bottom-40 left-0 p-3">SCROLL DOWN</p> */}
			</Reveal>
			{/* {props.children} */}
		</section>
	);
